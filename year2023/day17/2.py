...
- dynamically build vertices when adding neighbors to visit.  These
  vertices are position + last ten directions to get there.  Use
  the "min four, max ten" rules when determining neighbors

This is really slow, I think you could do better than tracking each coordinate
plus the whole ten previous moves
"""
...
class Vertex:
    ...
    h_score: int
    predecessor_key: str
    preceding_ten_directions: list[Direction]

    def __init__(self, row, col, g_score, h_score, predecessor_key, preceding_ten_directions):
        self.row = row
        self.col = col
        self.g_score = g_score
        self.h_score = h_score
        self.predecessor_key = predecessor_key
        self.preceding_ten_directions = preceding_ten_directions

    def __str__(self):
        return f"{self.coords()}: {self.g_score}"

    def key(self):
        return coordinate_direction_key(self.coords(), self.preceding_ten_directions)
...
while not vertex_keys_to_visit.empty():
    ...
    mandatory_direction = None
    opposite_of_preceding_direction = None

    if len(vertex.preceding_ten_directions) > 0:
        preceding_direction = vertex.preceding_ten_directions[-1]

        opposite_of_preceding_direction = opposite_direction[preceding_direction]

        n_steps_from_preceding_direction = 0

        for i in range(len(vertex.preceding_ten_directions)):
            if vertex.preceding_ten_directions[-(i + 1)] == preceding_direction:
                n_steps_from_preceding_direction += 1
            else:
                break

        if n_steps_from_preceding_direction < 4:
            mandatory_direction = preceding_direction

        if n_steps_from_preceding_direction == 10:
            forbidden_direction = preceding_direction
    ...
    for neighbor_vertex_coords in edges[vertex.coords()].keys():
        ...
        last_three_directions_to_neighbor = vertex.preceding_ten_directions[-9:] + [direction_to_neighbor]
        ...
        if mandatory_direction is not None and direction_to_neighbor != mandatory_direction:
            continue
        ...
        if neighbor_g_score < vertices[neighbor_key].g_score:
            ...
            vertices[neighbor_key].preceding_ten_directions = last_three_directions_to_neighbor
